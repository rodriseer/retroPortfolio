...
const viewportTabBaseClass =
  "rounded-[var(--os-radius)] border-2 px-2.5 py-1.5 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-retro-highlight focus-visible:ring-offset-2 focus-visible:ring-offset-retro-bg-dark sm:px-3 sm:text-[10px]";
...
function HomeView({ onViewProjects }: { onViewProjects: () => void }) {
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-5 overflow-hidden px-2 text-center sm:gap-6 md:gap-7">
      <div className="relative shrink-0">
        ...
        <p className="relative m-0 mt-3 font-retro text-3xl leading-none tracking-wide text-retro-highlight-bright retro-title-readable sm:text-5xl md:text-6xl lg:text-7xl">
          Rodrigo Seer
        </p>
      </div>
      <p className="relative z-[1] m-0 max-w-lg text-center text-sm os-body sm:text-base md:text-lg">
        Information Science student · I design and ship software, AI tools, and web systems that solve
        real outdoor and data problems.
      </p>
      <button
        type="button"
        onClick={onViewProjects}
        className="retro-cta-button relative z-[1] mt-1 shrink-0 rounded-[var(--os-radius)] border-2 border-retro-highlight/90 bg-retro-highlight/32 px-8 py-3.5 text-base text-retro-highlight-bright shadow-[0_4px_20px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] transition-[transform,background-color,border-color,box-shadow] duration-200 hover:scale-[1.015] hover:border-retro-highlight-bright hover:bg-retro-highlight/42 hover:shadow-[0_6px_24px_rgba(0,0,0,0.45)] active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-retro-highlight-bright focus-visible:ring-offset-2 focus-visible:ring-offset-retro-bg-dark md:px-10 md:py-4 md:text-lg"
      >
        View Projects
      </button>
    </div>
...
const PROFILE_BLOCKS: ProfileBlock[] = [
  {
    ...
    paragraphs: [
      ...
      "I like turning messy real-world data into tools people actually use — especially in outdoor and real-time environments.",
    ],
  },
  {
    ...
    kind: "paragraphs",
    paragraphs: [
      "I care about building things that people actually use in real life.",
      "Especially tools that help with real-world decisions — like surfing, cycling, or planning outdoors.",
    ],
  },
  {
    ...
    items: [
      ...
      "Systems that process live or messy data",
      "Clean UI with clear outputs",
      "API-driven tools",
    ],
  },
  {
    ...
    kind: "paragraphs",
    paragraphs: [
      "Grow Seer Labs into a platform for outdoor intelligence and continue building systems that turn data into action.",
    ],
  },
];
...
function ProfileView() {
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Profile — player data"
    >
      <ViewportTabStrip
        items={tabItems}
        activeId={panel}
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden pt-3" role="tabpanel">
        <section className="os-panel flex h-full min-h-0 flex-col overflow-hidden p-4 sm:p-5 md:p-6">
          <h3 className="m-0 shrink-0 border-b border-retro-border/60 pb-2.5 font-retro text-base tracking-wide text-retro-highlight-bright md:text-lg">
            {b.title}
          </h3>
          <PanelScrollArea key={panel} className="min-h-0 pt-3" showScrollHint>
            <div className="space-y-3 pb-2 font-sans text-[0.8125rem] leading-[1.65] text-retro-text/95 sm:text-sm sm:leading-[1.7]">
              {b.kind === "paragraphs" &&
...
const moduleActionsColClass =
  "w-full shrink-0 md:mt-0 md:flex md:w-[13.25rem] md:flex-col md:justify-end md:pt-0.5";
...
function ModuleMissionCard({
  ...
  const pad = dense ? "p-3.5 sm:p-4" : compact ? "p-5 md:p-6" : "p-6 md:p-7";
  const hasMeta = Boolean((m.highlights && m.highlights.length > 0) || m.impactLine);
  return (
    <div
      className={`group relative flex min-h-0 flex-col overflow-hidden rounded-[var(--os-radius)] border-2 border-retro-border/65 bg-[rgba(24,21,46,0.96)] shadow-[var(--panel-glow)] transition-[border-color,background-color] duration-200 ease-out hover:border-retro-highlight/55 hover:bg-[rgba(28,24,52,0.98)] ${pad}`}
    >
      <span
        className="pointer-events-none absolute left-0 top-0 h-full w-0.5 rounded-l-lg bg-retro-highlight opacity-0 transition-opacity duration-200 group-hover:opacity-100"
        aria-hidden
      />
      <div
        className={`relative flex min-h-0 flex-1 flex-col md:flex-row md:items-stretch ${dense ? "gap-3 md:gap-4" : "gap-5 md:gap-8"}`}
      >
        <div className={`flex min-h-0 min-w-0 flex-1 ${dense ? "gap-2 md:gap-3" : "gap-3 md:gap-4"}`}>
          <span
            className={`shrink-0 font-mono tabular-nums text-retro-muted/90 transition-colors group-hover:text-retro-highlight/70 ${dense ? "pt-0.5 text-[9px]" : "pt-1 text-[10px]"}`}
          >
            {slot}
          </span>
          <div className={`min-w-0 flex-1 ${dense ? "space-y-2" : "space-y-3"}`}>
            <div className={dense ? "space-y-1.5" : "space-y-2"}>
              <h3
                className={`m-0 font-retro leading-tight tracking-wide text-retro-highlight-bright retro-title-readable transition-colors ${dense ? "text-sm sm:text-[0.9375rem]" : "text-base sm:text-[1.0625rem]"}`}
              >
                {m.name}
              </h3>
              <p
                className={`m-0 font-sans text-retro-text/93 ${dense ? "line-clamp-4 text-[11px] leading-snug sm:line-clamp-5 sm:text-xs" : "text-[13px] leading-[1.72] sm:text-sm"}`}
              >
                {m.description}
              </p>
              {hasMeta && (
                <div
                  className={`space-y-0.5 border-l-2 border-retro-highlight/35 pl-2.5 ${dense ? "sm:pl-3" : "pl-3"}`}
                >
                  {m.highlights?.map((line) => (
                    <p
                      key={line}
                      className={`m-0 font-mono leading-snug text-retro-highlight/80 ${dense ? "text-[9px] sm:text-[10px]" : "text-[10px] sm:text-[11px]"}`}
                    >
                  ...
                  {(!m.highlights || m.highlights.length === 0) && m.impactLine && (
                    <p
                      className={`m-0 font-mono leading-relaxed text-retro-highlight/75 ${dense ? "text-[9px] sm:text-[10px]" : "text-[10px] sm:text-[11px]"}`}
                    >
                      {m.impactLine}
                    </p>
                  )}
            ...
            <ul className="m-0 flex list-none flex-wrap gap-1.5 p-0" aria-label="Technologies">
              {m.tags.map((tag) => (
                <li key={tag}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden pt-2.5" role="tabpanel">
        {registryTab === "platform" && (
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            <header className="shrink-0 border-b border-retro-border/50 pb-2">
              <div className="flex flex-wrap items-end justify-between gap-2">
                <div>
                  <p className="m-0 font-mono text-[8px] uppercase tracking-[0.22em] text-retro-muted/85">Registry</p>
                  <p className="m-0 mt-0.5 font-retro text-sm tracking-wide text-retro-highlight-bright md:text-base">
                    System deployables
                  </p>
                </div>
                <p className="m-0 font-mono text-[9px] tabular-nums text-retro-muted/80">
                  {String(TOTAL_PROJECT_SLOTS).padStart(2, "0")} modules
                </p>
              </div>
              <p className="m-0 mt-2 font-mono text-[8px] uppercase tracking-[0.18em] text-retro-muted/75">Primary platform</p>
            </header>

            <PanelScrollArea key="projects-platform" className="min-h-0 pt-2" showScrollHint>
              <article className="relative mb-1 flex flex-col rounded-[var(--os-radius)] border-2 border-retro-highlight/70 bg-gradient-to-b from-retro-highlight/[0.15] via-[rgba(24,21,46,0.98)] to-[rgba(18,16,36,0.99)] p-4 shadow-[var(--panel-glow)] sm:p-5 md:flex-row md:items-stretch md:gap-6 md:p-6">
                <div className="flex min-w-0 flex-1 flex-col gap-3">
                  <div className="flex flex-wrap gap-1.5">
                    {SEER_LABS.badgeLabels.map((b) => (
                      <span
                        key={b}
                        className="rounded-md border border-retro-highlight/45 bg-retro-highlight/[0.12] px-2 py-0.5 font-mono text-[8px] font-medium uppercase tracking-wider text-retro-highlight-bright sm:text-[9px]"
                      >
                        {b}
                      </span>
                    ))}
                  </div>
                  <h2 className="m-0 font-retro text-xl leading-tight tracking-wide text-retro-highlight-bright retro-title-readable sm:text-2xl md:text-3xl">
                    {SEER_LABS.name}
                  </h2>
                  <div className="space-y-2">
                    {SEER_LABS.descriptionLines.map((line) => (
                      <p
                        key={line}
                        className="m-0 text-[0.8125rem] leading-snug text-retro-text/95 sm:text-sm sm:leading-relaxed"
                      >
                        {line}
                      </p>
                    ))}
                  </div>
                  <p className="m-0 border-l-2 border-retro-highlight/40 pl-2.5 font-mono text-[10px] leading-snug text-retro-highlight/85">
                    {SEER_LABS.impactLine}
                  </p>
                  <ul className="m-0 flex list-none flex-wrap gap-1.5 p-0" aria-label="Stack">
                    {SEER_LABS.tags.map((t) => (
                      <li key={t}>
                        <span className="inline-block rounded-md border border-retro-border-inner/80 bg-black/40 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-retro-muted">
                          {t}
                        </span>
                      </li>
                    ))}
                ...
                <div className={`shrink-0 pt-3 md:pt-0 ${moduleActionsColClass}`}>
        ...
        {registryTab === "subsystems" && (
          <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
            <div className="shrink-0 border-b border-retro-border/50 pb-2">
              <span className="font-mono text-[9px] uppercase tracking-[0.16em] text-retro-muted/75">Subsystem registry</span>
              <span className="ml-2 font-retro text-xs tracking-wide text-retro-highlight/90">Seer Labs modules</span>
            </div>
            <PanelScrollArea key="projects-subsystems" className="min-h-0" showScrollHint>
              <div className="relative ml-0.5 space-y-4 border-l-2 border-retro-highlight/25 pl-4 pb-3 md:ml-1 md:pl-6">
                {SEER_LABS.modules.map((m, i) => (
                  <ModuleMissionCard key={m.name} m={m} slot={String(i + 1).padStart(2, "0")} compact />
                ))}
              </div>
            </PanelScrollArea>
          </div>
        ...
        {registryTab === "external" && (
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            <div className="shrink-0 border-b border-retro-border/50 pb-2">
              <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-retro-muted/75">Other projects</span>
              <span className="ml-2 font-sans text-[11px] text-retro-text/70">External modules</span>
            </div>
            <PanelScrollArea key="projects-external" className="min-h-0 pt-2" showScrollHint>
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden pt-2.5" role="tabpanel">
        {mainTab === "capabilities" && (
          <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
            <header className="shrink-0 border-b border-retro-border/50 pb-2">
              <p className="m-0 font-mono text-[8px] uppercase tracking-[0.2em] text-retro-muted/85">Character sheet</p>
              <p className="m-0 mt-0.5 font-retro text-sm tracking-wide text-retro-highlight-bright md:text-base">
                Capability matrix
              </p>
            </header>
            ...
            <PanelScrollArea key="skills-caps" className="min-h-0" showScrollHint>
              <div className="space-y-3 pb-3">
                {SKILL_GROUPS.map((group) => (
                  <div
                    key={group.category}
                    className="os-panel flex flex-col gap-2.5 p-3 sm:p-4 md:p-4"
                  >
                    <h3 className="m-0 border-b border-retro-border/45 pb-2 font-retro text-sm tracking-wide text-retro-highlight-bright">
                      {group.category}
                    </h3>
          ...
          <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
            <header className="shrink-0 border-b border-retro-border/50 pb-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <h3 className="m-0 font-retro text-sm text-retro-highlight-bright md:text-base">Achievements</h3>
                <span className="font-mono text-[8px] uppercase tracking-wider text-retro-muted/85">Unlocked</span>
              </div>
            </header>
            <PanelScrollArea key="skills-ach" className="min-h-0" showScrollHint>
              <ul className="m-0 grid list-none grid-cols-1 gap-2 p-0 pb-3 sm:grid-cols-3 sm:gap-2 md:gap-3">
                {ACHIEVEMENTS.map((a) => (
                  <li
                    key={a.title}
                    className="flex min-w-0 gap-2 rounded-[var(--os-radius)] border-2 border-retro-border/60 bg-[rgba(20,18,40,0.92)] p-3 sm:p-3.5"
                  >
                    <span className="shrink-0 select-none text-sm leading-none text-emerald-400/85" aria-hidden>
                      ✓
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="m-0 font-retro text-xs text-retro-highlight sm:text-sm">{a.title}</p>
                      <p className="m-0 mt-1.5 text-[10px] leading-snug text-retro-text/90 sm:text-[11px]">{a.detail}</p>
                    </div>
                  </li>
                ))}
              </ul>
...
function FilesView() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden font-mono text-xs sm:text-sm">
      <p className="mb-2 shrink-0 text-[10px] uppercase tracking-wider text-retro-muted/85">
        Volume SYS: — /user/rodrigo/
      </p>
      <PanelScrollArea className="min-h-0" showScrollHint>
        <ul className="m-0 list-none space-y-2 p-0 pb-2">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-center overflow-hidden py-1">
      <div className="flex max-h-full min-h-0 flex-col overflow-hidden rounded-[var(--os-radius)] border-2 border-retro-border/70 bg-[rgba(18,16,36,0.88)] shadow-[var(--panel-glow)]">
        <div className="shrink-0 border-b border-retro-border/60 bg-retro-panel-solid/80 px-4 py-2.5 font-retro text-[10px] uppercase tracking-[0.2em] text-retro-highlight">
        ...
        <ul className="m-0 flex list-none flex-col divide-y divide-retro-border/40 p-0">
          {items.map((item) => (
            ...
            >
              <span className="shrink-0 font-retro text-[11px] uppercase tracking-wider text-retro-muted sm:text-xs">
                {item.label}
              </span>
              <a
                href={item.href}
